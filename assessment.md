...
The local quality baseline is production-grade for this single-user tool. Ruff, strict mypy, 73 non-live tests, a 75% coverage floor, Bandit, frozen dependency auditing, migration round trips, CLI tests, Streamlit smoke tests, and tracked-private-file checks pass. CI actions use Node.js 24 releases pinned to full commit SHAs. Mocked tests cover the critical browser and recovery branches. Live YouTube behavior still requires controlled operator validation because it cannot be proven by CI.
